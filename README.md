...
## Pre-requisites needed to run the test suite:
  ...
  1. Python environment (Python 3.8 +) - https://www.python.org/downloads/
  2. IDE (Pycharm or any other IDE of your preference)
...
![project_structure](docs/project_structure.jpeg)
...
running the above command will trigger behave to read the feature files and execute the step definitions. When the execution 
is completed an allure report will be created under the folder `/reports`. The reports can be viewed either in JSON format or served
using `allure`. 

![screen recording](docs/screen_record_behave.gif)
...
```
![allure-report image](docs/allure-report.png)
...
![unittests-report image](docs/unit_test_report.png)
